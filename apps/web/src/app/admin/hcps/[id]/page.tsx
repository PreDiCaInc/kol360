    ...
    <RequireAuth allowedRoles={['PLATFORM_ADMIN', 'CLIENT_ADMIN']}>
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle>Disease Area Scores</CardTitle>
              <CardDescription>Current influence scores by disease area</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">Conferences</TableHead>
                    ...
                    {hcp.diseaseAreaScores.map((score) => (
                      <TableRow key={score.id}>
                        ...
                        <TableCell className="text-right">
                          {(score as unknown as { publicationsScore?: number }).publicationsScore?.toFixed(1) || '—'}
                        </TableCell>
                        <TableCell className="text-right">
                          {(score as unknown as { clinicalTrialsScore?: number }).clinicalTrialsScore?.toFixed(1) || '—'}
                        </TableCell>
                        <TableCell className="text-right">
                          {(score as unknown as { conferenceScore?: number }).conferenceScore?.toFixed(1) || '—'}
                        </TableCell>
                        <TableCell className="text-right">
                          <Badge variant="secondary">
                            {(score as unknown as { compositeScore?: number }).compositeScore?.toFixed(1) || '—'}
                          </Badge>
                        </TableCell>
                      </TableRow>
                    ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Recent campaign participations</CardDescription>
            </CardHeader>
            <CardContent>
              {hcp.campaignHcps && hcp.campaignHcps.length > 0 ? (
                <div className="space-y-3">
                  {hcp.campaignHcps.map((ch) => (
                    ...
                    >
                      <div>
                        <p className="font-medium text-sm">{ch.campaign.name}</p>
                      </div>
                      <Badge
                        variant={
              ...
              {hcp.campaignScores && hcp.campaignScores.length > 0 && (
                <div className="mt-6 pt-6 border-t">
                  <h4 className="text-sm font-medium mb-3">Campaign Scores</h4>
                  <div className="space-y-2">
